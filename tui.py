...
class TaskAddModal(ModalScreen):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "submit":
            ...
            if title:
                self.dismiss((title, due, priority))
            else:
                self.dismiss()
        else:
            self.dismiss()
...
class GoalAddModal(ModalScreen):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "submit":
            ...
            if title: self.dismiss((title, desc, bn, bd))
            else: self.dismiss()
        else:
            self.dismiss()
...
class ProjectAddModal(ModalScreen):
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "submit":
            ...
            if title: self.dismiss((title, desc, goal))
            else: self.dismiss()
        else:
            self.dismiss()
...
class Dashboard(Screen):
    ...
    def on_mount(self) -> None:
        ...
        tasks_table.add_columns("ID", "Title", "Due", "Priority")
    ...
    def refresh_data(self) -> None:
        ...
        for t in data_store.tasks:
            if t.status == "todo":
                tasks_table.add_row(t.id, t.title, t.due_date or "-", t.priority)
        badges_table = self.query_one("#dashboard-badges", DataTable)
        badges_table.clear()
...
class GoalsProjectsScreen(Screen):
    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("Goals & Projects (g/p: New, e: Edit, x: Delete, d: Done)", classes="section-title")
        with Horizontal():
    ...
    def on_mount(self) -> None:
        ...
        gt.add_columns("ID", "Title", "Status")
        gt.cursor_type = "row"
        
        pt = self.query_one("#projects-table", DataTable)
        pt.add_columns("ID", "Title", "Status")
        pt.cursor_type = "row"
    ...
    def refresh_tables(self) -> None:
        gt = self.query_one("#goals-table", DataTable)
        gt.clear()
        for g in data_store.goals: gt.add_row(g.id, g.title, g.status)
        pt = self.query_one("#projects-table", DataTable)
        pt.clear()
        for p in data_store.projects: pt.add_row(p.id, p.title, p.status)
...
class TasksScreen(Screen):
    def compose(self) -> ComposeResult:
        yield Header()
        yield Label("All Tasks (n: New, d: Done, e: Edit, x: Delete)", classes="section-title")
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("ID", "Title", "Due", "Priority", "Status")
        self.refresh_table()
    ...
    def add_task_callback(self, result: tuple):
        if result:
            title, due, priority = result
            commands.task_add(title, due_date=due, priority=priority)
            self.refresh_table()

    def edit_task_callback(self, task_id, res):
        if res:
            commands.task_edit(task_id, title=res[0], due_date=res[1], priority=res[2])
            self.refresh_table()
    ...
    def refresh_table(self):
        ...
        for t in sorted(data_store.tasks, key=lambda x: (x.status == "done", x.priority != "high", x.priority != "medium")):
            table.add_row(t.id, t.title, t.due_date or "-", t.priority, t.status)
